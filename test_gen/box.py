...
with open("ispd/tests/box/box_big.in", "w+") as f:
    f.write(F"{dim}\n")
...
with open("ispd/tests/box/box_big.out", "w+") as f:
    ...
    for i in range(0, 3):
        for j in range(0, 3):
            for k in range(0, 3):
                ...
                x_shape = int(vol_x * inv_sampling_step / (3 * 2))
                y_shape = int(vol_y * inv_sampling_step / (3 * 2))
                z_shape = int(vol_z * inv_sampling_step / (3 * 2))
